...
public class BackendAPI {
    ...
    public static final String URL = "http://192.168.178.22:3000";
    private static String TOKEN = null;
    public static String userId = null;
    public static Optional<JSONObject> last_result;
    public static ArrayList<String> myRoles = new ArrayList<>();
    ...
    private static Optional<JSONObject> _send(PropertyConstructor properties, String endpoint, String method) {
        ...
        if (properties != null) //Meaning you got no parameters
            if(properties.getProperty("userId") != null && properties.getProperty("coachId") != null) {
                String s = properties.construct().replace("\\\"", "\"").replace("\"{", "{").replace("}\"", "}");
                Log.d("BACKEND", s);
                data = s.getBytes(StandardCharsets.UTF_8);
            }
            else data = properties.construct().getBytes(StandardCharsets.UTF_8);
    ...
    public static boolean Authenticate(String email, String password) {
        ...
        Optional<JSONObject> response = Send(new PropertyConstructor()
                ...
                .addProperty("password", password), "login", "POST");
        if (!response.isPresent()) return false;

        try {
            //Initialize token from sign in
            String t = response.get().getString("token");
            TOKEN = t;
        } catch (JSONException e) {
            //e.printStackTrace();
            TOKEN = null;
